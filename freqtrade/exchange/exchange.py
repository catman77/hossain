...
class Exchange:
    ...
    def get_markets(self,base_currencies: Optional[List[str]] = None,quote_currencies: Optional[List[str]] = None,spot_only: bool = False,margin_only: bool = False,futures_only: bool = False,tradable_only: bool = True,active_only: bool = False) -> Dict[str, Any]:
    ...
    def market_is_tradable(self, market: Dict[str, Any]) -> bool:
        return (market.get("quote", None) is not None
    ...
    def validate_stakecurrency(self, stake_currency: str) -> None:
        if not self._markets:
            raise OperationalException("Could not load markets, therefore cannot start. "
                "Please investigate the above error for more details.")
        quote_currencies = self.get_quote_currencies()
        if stake_currency not in quote_currencies:
            raise ConfigurationError(f"{stake_currency} is not available as stake on {self.name}. "
    ...
    def validate_pairs(self, pairs: List[str]) -> None:
        ...
        for pair in extended_pairs:
            if self.markets and pair not in self.markets:
                raise OperationalException(f"Pair {pair} is not available on {self.name} {self.trading_mode.value}. "
            ...
            if (self._config["stake_currency"]
                and self.get_pair_quote_currency(pair) != self._config["stake_currency"]):
                invalid_pairs.append(pair)
        if invalid_pairs:
            raise OperationalException(f"Stake-currency '{self._config['stake_currency']}' not compatible with "
    ...
    def _get_stop_limit_rate(self, stop_price: float, order_types: Dict, side: str) -> float:
        ...
        if bad_stop_price:
            raise InvalidOrderException("In stoploss limit order, stop price should be more than limit price. "
    ...
    def get_balances(self) -> dict:
        try:
            ...
            raise TemporaryError(f"Could not get balance due to {e.__class__.__name__}. Message: {e}"
            ) from e
        except ccxt.BaseError as e:
            raise OperationalException(e) from e
    ...
    def fetch_l2_order_book(self, pair: str, limit: int = 100) -> OrderBook:
        ...
        try:
            ...
            raise OperationalException(f"Exchange {self._api.name} does not support fetching order book. Message: {e}"
            ) from e
        except ccxt.DDoSProtection as e:
            raise DDosProtection(e) from e
        except (ccxt.OperationFailed, ccxt.ExchangeError) as e:
    ...
    def _get_rate_from_ob(self,pair: str,side: EntryExit,order_book: OrderBook,name: str,price_side: BidAsk,order_book_top: int) -> float:
        ...
        try:
            ...
            logger.warning(f"{pair} - {name} Price at location {order_book_top} from orderbook "
    ...
    def get_trades_for_order(self, order_id: str, pair: str, since: datetime, params: Optional[Dict] = None) -> List:
        ...
        try:
            ...
            raise TemporaryError(f"Could not get trades due to {e.__class__.__name__}. Message: {e}"
            ) from e
        except ccxt.BaseError as e:
            raise OperationalException(e) from e
    ...
    def get_fee(self,symbol: str,type: str = "",side: str = "",amount: float = 1,price: float = 1,taker_or_maker: MakerTaker = "maker") -> float:
        ...
        try:
            ...
            return self._api.calculate_fee(symbol=symbol,type=type,side=side,
                amount=amount,
                price=price,
    ...
    def _get_funding_fees_from_exchange(self, pair: str, since: Union[datetime, int]) -> float:
        ...
        try:
            ...
            raise TemporaryError(f"Could not get funding fees due to {e.__class__.__name__}. Message: {e}"
            ) from e
        except ccxt.BaseError as e:
            raise OperationalException(e) from e
