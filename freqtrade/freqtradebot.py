...
class FreqtradeBot(LoggingMixin):
    ...
    def update_trades_without_assigned_fees(self) -> None:
        ...
        for trade in trades:
            if not trade.is_open and not trade.fee_updated(trade.exit_side):
                ...
                if order:
                    logger.info(f"Updating {trade.exit_side}-fee on trade {trade}"
    ...
    def execute_entry(self, pair: str, stake_amount: float, price: Optional[float] = None,*, is_short: bool = False, ordertype: Optional[str] = None,enter_tag: Optional[str] = None,trade: Optional[Trade] = None,mode: EntryExecuteMode = "initial",leverage_: Optional[float] = None) -> bool:
        ...
        order_type = ordertype or self.strategy.order_types["entry"]   #! get entery 'limit'  or 'market' from strategy
    ...
    def exit_positions(self, trades: List[Trade]) -> int:
        ...
        for trade in trades:
            if ( not trade.has_open_orders
                ...
                logger.warning(f"Not enough {trade.safe_base_currency} in wallet to exit {trade}. "
                    "Trying to recover.")
    ...
    def _check_and_execute_exit(self, trade: Trade, exit_rate: float, enter: bool, exit_: bool, exit_tag: Optional[str]) -> bool:
        ...
        exits: List[ExitCheckTuple] = self.strategy.should_exit(trade,exit_rate,datetime.now(timezone.utc),
            enter=enter,exit_=exit_,force_stoploss=self.edge.get_stoploss(trade.pair) if self.edge else 0)
    ...
    def execute_trade_exit(self,trade: Trade,limit: float,exit_check: ExitCheckTuple,*,exit_tag: Optional[str] = None,ordertype: Optional[str] = None,sub_trade_amt: Optional[float] = None) -> bool:
        ...
        if exit_check.exit_type == ExitType.EMERGENCY_EXIT:
            order_type = self.strategy.order_types.get("emergency_exit", "market")  #!  check exit type defined in strategy for stop loss or emergency 'limit'  or 'market'

        amount = self._safe_exit_amount(trade, trade.pair, sub_trade_amt or trade.amount)

        time_in_force = self.strategy.order_time_in_force["exit"]

        if (exit_check.exit_type != ExitType.LIQUIDATION  and not sub_trade_amt  and not strategy_safe_wrapper(self.strategy.confirm_trade_exit, default_retval=True)(
                pair=trade.pair,
                trade=trade,
                order_type=order_type,
                amount=amount,
