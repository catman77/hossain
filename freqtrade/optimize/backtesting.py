...
class Backtesting:
    ...
    def load_bt_data_detail(self) -> None:
        ...
        if self.trading_mode == TradingMode.FUTURES:
            ...
            if unavailable_pairs:
                raise OperationalException(f"Pairs {', '.join(unavailable_pairs)} got no leverage tiers available. "
    ...
    def _get_adjust_trade_entry_for_candle(self, trade: LocalTrade, row: Tuple, current_time: datetime) -> LocalTrade:
        ...
        if stake_amount is not None and stake_amount < 0.0:
            amount = amount_to_contract_precision(abs(float(FtPrecise(stake_amount) * FtPrecise(trade.amount) / FtPrecise(trade.stake_amount)
    ...
    def _process_exit_order(self, order: Order, trade: LocalTrade, current_time: datetime, row: Tuple, pair: str):
