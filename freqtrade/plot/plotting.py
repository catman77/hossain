...
def generate_candlestick_graph(pair: str,data: pd.DataFrame,trades: Optional[pd.DataFrame] = None,*,indicators1: Optional[List[str]] = None,indicators2: Optional[List[str]] = None,plot_config: Optional[Dict[str, Dict]] = None) -> go.Figure:
...
def load_and_plot_trades(config: Config):
    ...
    for pair, data in plot_elements["ohlcv"].items():
        ...
        fig = generate_candlestick_graph(pair=pair,data=df_analyzed,trades=trades_pair,indicators1=config.get("indicators1", []),indicators2=config.get("indicators2", []),plot_config=strategy.plot_config if hasattr(strategy, "plot_config") else {})

        store_plot_file(fig,filename=generate_plot_filename(pair, config["timeframe"]),directory=config["user_data_dir"] / "plot")
