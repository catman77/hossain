...
class coinex(Exchange, ImplicitAPI):
    ...
    def parse_ticker(self, ticker: dict, market: Market = None) -> Ticker:
        ...
        last_price = self.safe_string(ticker, 'last')
        return self.safe_ticker({
            ...
            'quoteVolume': float(base_volume) * float(last_price) ,
            'info': ticker,
        }, market)
    ...
    def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        response = self.v2PublicGetFuturesTicker(request)
        response1 = self.v2PublicGetFuturesDepth(request1)

        data = self.safe_list(response, 'data', [])
        result = self.safe_dict(data, 0, {})
        ticker = self.parse_ticker(result, market)

        data1 = self.safe_dict(response1, 'data', {})
        depth = self.safe_dict(data1, 'depth', {})
        timestamp = self.safe_integer(depth, 'updated_at')
        orderbook = self.parse_order_book(depth, symbol, timestamp)

        bids = orderbook['bids'][0][0]
    ...
    def fetch_order_book(self, symbol: str, limit: Int = 5, params={}):
        """
        no params{}
        ...
        response = None
        response = self.v2PublicGetFuturesDepth(request)
        data = self.safe_dict(response, 'data', {})
    ...
    def fetch_trades(self, symbol: str, since: Int = None, limit: Int = None, params={}) -> List[Trade]:
        ...
        params['last_id]  The starting point of the query TxID, 0 means to acquire from the latest record
        """
        self.load_markets()
        market = self.market(symbol)
        request: dict = {
        ...
        if 'last_id' in params:
            request['last_id'] = params['last_id']
        response = None
        response = self.v2PublicGetFuturesDeals(request)
    ...
    def fetch_trading_fee(self, symbol: str, params={}) -> TradingFeeInterface:
        ...
        response = None
        response = self.v2PublicGetFuturesMarket(request)
        data = self.safe_list(response, 'data', [])
        result = self.safe_dict(data, 0, {})
    ...
    def fetch_trading_fees(self, params={}) -> TradingFees:
        ...
        if 'market' in params:
            market = self.market(params['market'])
            request['market'] = request['id']
        response = None
        response = self.v2PublicGetFuturesMarket(request)
        data = self.safe_list(response, 'data', [])
        result: dict = {}
    ...
    def fetch_ohlcv(self, symbol: str, timeframe='1m', since: Int = None, limit: Int = None, params={}) -> List[list]:
        ...
        params['price_type]:mark  fetch mark_price ohlcv
        """
        self.load_markets()
        market = self.market(symbol)
        if 'price_type' in params:
            if params['price_type'] =='mark':
                request['price_type'] = 'mark_price'
        request: dict = {
            'market': market['id'],
    ...
    def create_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        ...
        request['client_id'] = self.default_id + self.uuid16()
        ...
        data = self.safe_dict(response, 'data', {})
        corder:dict = self.parse_order(data, market)
        corder['status'] = 'open'
        return corder
    ...
    def create_stop_loss_order(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, stopLossPrice: Num = None, params={}):
        ...
        request['amount'] = self.amount_to_precision(symbol, amount)
        request['client_id'] = self.default_id + self.uuid16()
        request['trigger_price_type'] = 'latest_price'
        if type == 'limit':
            request['price'] = self.price_to_precision(symbol, price)
    ...
    def close_coinex_position(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        """
        no params{}
        close a position like create order
        """
        request:dict = {}
        response:dict = None
        self.load_markets()
        ...
        data = self.safe_dict(response, 'data', {})
        corder = self.parse_order(data, market)
        corder['status'] = 'closed'
        return corder
    ...
    def cancel_order(self, id: str, symbol: Str = None, params={}):
        ...
        response = None
        response = self.v2PrivatePostFuturesCancelOrder(request)

        data = self.safe_dict(response, 'data', {})
        corder:dict = self.parse_order(data, market)
        corder['status'] = 'canceled'
        return corder
    ...
    def fetch_order(self, id: str, symbol: Str = None, params={}):
        ...
        response = None
        response = self.v2PrivateGetFuturesOrderStatus(request)
    ...
    def fetch_orders(self, symbol: Str = None, since: Int = None, limit: Int = None, params={}):
        ...
        open_orders = self.fetch_open_orders(symbol= symbol , since= since , limit= limit , params= params)
        closed_orders = self.fetch_closed_orders(symbol= symbol , since= since , limit= limit , params= params)
    ...
    def fetch_positions(self, symbols: Strings = None, params={}):
        ...
        if symbols is None:
            ...
            response = self.v2PrivateGetFuturesPendingPosition(request)
        ...
        if symbols is not None:
            if isinstance(symbols , list):
                ...
                if len(positions) > 0:
                    for position in positions:
                        results.append(self.parse_position(position , market))
                    return results



    def fetch_closed_positions(self, symbols: Strings = None, params={}):
        """
        params = {'limit':int}
        ...
        if symbols is None:
            results:list = []
            response = self.v2PrivateGetFuturesFinishedPosition(request)
            positions = self.safe_list(response, 'data', [])
    ...
    def create_order_request(self, symbol: str, type: OrderType, side: OrderSide, amount: float, price: Num = None, params={}):
        market = self.market(symbol)
        swap = market['swap']
        clientOrderId = self.default_id + self.uuid16()
