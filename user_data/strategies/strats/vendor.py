...
buy_params = {'base_nb_candles_buy': 17, 'ewo_high': 3.33, 'ewo_high_2': -0.444, 'ewo_low': -12.788, 'lookback_candles': 1, 'low_offset': 0.985, 'low_offset_2': 0.935, 'profit_threshold': 1.016, 'clucha_rocr_1h': 0.47782, 'clucha_bbdelta_close': 0.02206, 'clucha_bbdelta_tail': 1.02515, 'clucha_close_bblower': 0.03669, 'clucha_closedelta_close': 0.04401, 'base_nb_candles_buy_zema': 25, 'base_nb_candles_buy_zema2': 53, 'low_offset_zema': 0.958, 'low_offset_zema2': 0.961, 'base_nb_candles_buy_ema': 9, 'base_nb_candles_buy_ema2': 75, 'low_offset_ema': 1.067, 'low_offset_ema2': 0.973, 'high_offset_sell_ema': 0.994, 'rsi_buy': 71}

sell_params = {'base_nb_candles_sell': 8, 'high_offset': 1.01, 'high_offset_2': 1.406, 'ProfitLoss1': 0.005, 'ProfitLoss2': 0.021, 'ProfitMargin1': 0.018, 'ProfitMargin2': 0.051, 'pHSL': -0.15}
...
class vn(IStrategy):
    ...
    ProfitLoss2 = DecimalParameter(0.01, 0.025, default=sell_params['ProfitLoss2'], decimals=3, space='sell', optimize=True)
    ...
    timeperiods = [180 // timeframe_minutes, 360 // timeframe_minutes, 420 // timeframe_minutes, 560 // timeframe_minutes]
    ...
    def confirm_trade_exit(self, pair: str, trade: Trade, order_type: str, amount: float, rate: float, time_in_force: str, exit_reason: str, current_time: datetime, **kwargs) -> bool:
        ...
        if hasattr(last_candle, 'sell_tag') and str(last_candle.sell_tag) != 'nan':
            if exit_reason == 'exit_signal':
                exit_reason = last_candle.sell_tag
            else:
                exit_reason += last_candle.sell_tag
        trade.sell_reason = f'{trade.buy_tag}->{exit_reason}'
        return True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['rsi_fast'] < 35) & (dataframe['rsi_36'] < 0.45) & (dataframe['close'] < dataframe[f'ma_buy_{self.base_nb_candles_buy.value}'] * self.low_offset.value) & (dataframe['EWO'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_buy.value) & (dataframe['volume'] > 0) & (dataframe['close'] < dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value), ['enter_long', 'enter_tag']] = (1, 'ewo1')
        ...
        dataframe.loc[(dataframe['volume'] > 0) & dataframe['rocr_1h'].gt(self.clucha_rocr_1h.value) & (dataframe['lower'].shift().gt(0) & dataframe['bbdelta'].gt(dataframe['ha_close'] * self.clucha_bbdelta_close.value) & dataframe['ha_closedelta'].gt(dataframe['ha_close'] * self.clucha_closedelta_close.value) & dataframe['tail'].lt(dataframe['bbdelta'] * self.clucha_bbdelta_tail.value) & dataframe['ha_close'].lt(dataframe['lower'].shift()) & dataframe['ha_close'].le(dataframe['ha_close'].shift()) | (dataframe['ha_close'] < dataframe['hema_slow']) & (dataframe['ha_close'] < self.clucha_close_bblower.value * dataframe['lower'])), ['enter_long', 'enter_tag']] = (1, 'clucHA')
        # This does not fit well the protections for EWO buys.
        dataframe.loc[((dataframe['close'] < dataframe['zema_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['zema_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh'])) & (dataframe['volume'] > 0) & (dataframe['long_term_price_warning'] < 1) & (dataframe['trendline'] > 0.996) & (dataframe['btc_rsi'] > 0.49) & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_sell'] * self.high_offset_sell_ema.value) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < 35) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & ((dataframe['close'] < dataframe['ema_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['EWO'] < -19.632) | (dataframe['EWO'] > 2.615) & (dataframe['rsi'] < 60)) | (dataframe['close'] < dataframe['ema_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['EWO'] < -19.955) | (dataframe['EWO'] > 2.188) & (dataframe['rsi'] < 45))), ['enter_long', 'enter_tag']] = (1, 'zema')
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        conditions.append((dataframe['close'] > dataframe['sma_9']) & (dataframe['close'] > dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset_2.value) & (dataframe['rsi'] > 50) & (dataframe['volume'] > 0) & (dataframe['uptrend'] == 1) & (dataframe['rsi_fast'] > dataframe['rsi_slow']) | (dataframe['close'] < dataframe['hma_50']) & (dataframe['close'] > dataframe[f'ma_sell_{self.base_nb_candles_sell.value}'] * self.high_offset.value) & (dataframe['volume'] > 0) & (dataframe['uptrend'] == 1) & (dataframe['rsi_fast'] > dataframe['rsi_slow']))
        if conditions:
...
def williams_r(dataframe: DataFrame, timeperiod: int=14) -> Series:
    ...
    WR = Series((highest_high - dataframe['close']) / (highest_high - lowest_low), name=f'{timeperiod} Williams %R')
    return WR * -100
