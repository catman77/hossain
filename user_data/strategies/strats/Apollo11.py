...
class A11(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['s3_fast_ma'] = ta.EMA(dataframe['volume'] * dataframe['close'], self.s3_ma_fast) / ta.EMA(dataframe['volume'], self.s3_ma_fast)
        dataframe['s3_slow_ma'] = ta.EMA(dataframe['volume'] * dataframe['close'], self.s3_ma_slow) / ta.EMA(dataframe['volume'], self.s3_ma_slow)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_signal_3:
            conditions = [dataframe['low'] < dataframe['s3_bb_lowerband'], dataframe['high'] > dataframe['s3_slow_ma'], dataframe['high'] < dataframe['s3_ema_long'], dataframe['volume'] > 0]
