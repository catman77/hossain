...
class newstrategy51(IStrategy):
    ...
    # value loaded from strategy
    # value loaded from strategy
    buy_params = {'bbdelta_close': 0.00082, 'bbdelta_tail': 0.85788, 'close_bblower': 0.00128, 'closedelta_close': 0.00987, 'low_offset': 0.991, 'rocr1_1h': 0.9346, 'rocr_1h': 0.65666, 'base_nb_candles_buy': 12, 'buy_bb_delta': 0.025, 'buy_bb_factor': 0.995, 'buy_bb_width': 0.095, 'buy_bb_width_1h': 1.074, 'buy_cci': -116, 'buy_cci_length': 25, 'buy_closedelta': 15.0, 'buy_clucha_bbdelta_close': 0.049, 'buy_clucha_bbdelta_tail': 1.146, 'buy_clucha_close_bblower': 0.018, 'buy_clucha_closedelta_close': 0.017, 'buy_clucha_rocr_1h': 0.526, 'buy_ema_diff': 0.025, 'buy_rmi': 49, 'buy_rmi_length': 17, 'buy_roc_1h': 10, 'buy_srsi_fk': 32}
    ...
    # value loaded from strategy
    # value loaded from strategy
    sell_params = {'high_offset': 1.012, 'high_offset_2': 1.016, 'sell_deadfish_bb_factor': 1.089, 'sell_deadfish_bb_width': 0.11, 'sell_deadfish_profit': -0.107, 'sell_deadfish_volume_factor': 1.761, 'base_nb_candles_sell': 22, 'pHSL': -0.397, 'pPF_1': 0.012, 'pPF_2': 0.07, 'pSL_1': 0.015, 'pSL_2': 0.068, 'sell_bbmiddle_close': 1.09092, 'sell_fisher': 0.46406, 'sell_trail_down_1': 0.03, 'sell_trail_down_2': 0.015, 'sell_trail_profit_max_1': 0.4, 'sell_trail_profit_max_2': 0.11, 'sell_trail_profit_min_1': 0.1, 'sell_trail_profit_min_2': 0.04}
    # ROI table:
    minimal_roi = {'0': 0.276, '32': 0.105, '88': 0.037, '208': 0}
    #dca
    ...
    sell_deadfish_bb_factor = DecimalParameter(0.9, 1.2, default=1.0, space='sell', optimize=is_optimize_deadfish)
    ...
    pHSL = DecimalParameter(-0.5, -0.04, default=-0.08, decimals=3, space='sell', optimize=False, load=True)
    # profit threshold 1, trigger point, SL_1 is used
    pPF_1 = DecimalParameter(0.008, 0.02, default=0.016, decimals=3, space='sell', optimize=False, load=True)
    pSL_1 = DecimalParameter(0.008, 0.02, default=0.011, decimals=3, space='sell', optimize=False, load=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # from BinH
        dataframe.loc[(dataframe[f'rmi_length_{self.buy_rmi_length.value}'] < self.buy_rmi.value) & (dataframe[f'cci_length_{self.buy_cci_length.value}'] <= self.buy_cci.value) & (dataframe['srsi_fk'] < self.buy_srsi_fk.value) & (dataframe['bb_delta'] > self.buy_bb_delta.value) & (dataframe['bb_width'] > self.buy_bb_width.value) & (dataframe['closedelta'] > dataframe['close'] * self.buy_closedelta.value / 1000) & (dataframe['close'] < dataframe['bb_lowerband3'] * self.buy_bb_factor.value) & (dataframe['roc_1h'] < self.buy_roc_1h.value) & (dataframe['bb_width_1h'] < self.buy_bb_width_1h.value), ['enter_long', 'enter_tag']] = (1, 'DIP signal')  # from BinH
        dataframe.loc[(dataframe['bb_delta'] > self.buy_bb_delta.value) & (dataframe['bb_width'] > self.buy_bb_width.value) & (dataframe['closedelta'] > dataframe['close'] * self.buy_closedelta.value / 1000) & (dataframe['close'] < dataframe['bb_lowerband3'] * self.buy_bb_factor.value) & (dataframe['roc_1h'] < self.buy_roc_1h.value) & (dataframe['bb_width_1h'] < self.buy_bb_width_1h.value), ['enter_long', 'enter_tag']] = (1, 'Break signal')
        dataframe.loc[(dataframe['rocr_1h'] > self.buy_clucha_rocr_1h.value) & (dataframe['bb_lowerband2_40'].shift() > 0) & (dataframe['bb_delta_cluc'] > dataframe['ha_close'] * self.buy_clucha_bbdelta_close.value) & (dataframe['ha_closedelta'] > dataframe['ha_close'] * self.buy_clucha_closedelta_close.value) & (dataframe['tail'] < dataframe['bb_delta_cluc'] * self.buy_clucha_bbdelta_tail.value) & (dataframe['ha_close'] < dataframe['bb_lowerband2_40'].shift()) & (dataframe['close'] > dataframe['sup_level_1h'] * 0.88) & (dataframe['ha_close'] < dataframe['ha_close'].shift()), ['enter_long', 'enter_tag']] = (1, 'cluc_HA')
        dataframe.loc[(dataframe['ema_200'] > dataframe['ema_200'].shift(12) * 1.01) & (dataframe['ema_200'] > dataframe['ema_200'].shift(48) * 1.07) & dataframe['bb_lowerband2_40'].shift().gt(0) & dataframe['bb_delta_cluc'].gt(dataframe['close'] * 0.056) & dataframe['closedelta'].gt(dataframe['close'] * 0.01) & dataframe['tail'].lt(dataframe['bb_delta_cluc'] * 0.5) & dataframe['close'].lt(dataframe['bb_lowerband2_40'].shift()) & dataframe['close'].le(dataframe['close'].shift()) & (dataframe['close'] > dataframe['ema_50'] * 0.912), ['enter_long', 'enter_tag']] = (1, 'NFIX39')
        dataframe.loc[(dataframe['close'] > dataframe['sup_level_1h'] * 0.72) & (dataframe['close'] < dataframe['ema_16'] * 0.982) & (dataframe['EWO'] < -10.0) & (dataframe['cti'] < -0.9), ['enter_long', 'enter_tag']] = (1, 'NFIX29')
        dataframe.loc[(dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.buy_ema_diff.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb_lowerband2'] * self.buy_bb_factor.value) & (dataframe['closedelta'] > dataframe['close'] * self.buy_closedelta.value / 1000), ['enter_long', 'enter_tag']] = (1, 'local_uptrend')  # 0.053)
        # -0.8)
        #(dataframe['cmf'] > -0.20) & # povodne som mal -0.10
        dataframe.loc[(dataframe['close'] < dataframe['vwap_low']) & (dataframe['tcp_percent_4'] > 0.053) & (dataframe['cti'] < -0.8) & (dataframe['rsi'] < 35) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'vwap')
        dataframe.loc[(dataframe['bb_width_1h'] > 0.131) & (dataframe['r_14'] < -51) & (dataframe['r_84_1h'] < -70) & (dataframe['cti'] < -0.845) & (dataframe['cti_40_1h'] < -0.735) & (dataframe['close'].rolling(48).max() >= dataframe['close'] * 1.1) & (dataframe['btc_close'].rolling(24).max() >= dataframe['btc_close'] * 1.03), ['enter_long', 'enter_tag']] = (1, 'insta_signal')
        dataframe.loc[(dataframe['close'] < dataframe['ema_16'] * self.buy_44_ma_offset) & (dataframe['ewo'] < self.buy_44_ewo) & (dataframe['cti'] < self.buy_44_cti) & (dataframe['r_480_1h'] < self.buy_44_r_1h) & (dataframe['volume'] > 0), ['enter_long', 'enter_tag']] = (1, 'NFINext44')
        #(dataframe['safe_dump_50_1h'])  
        dataframe.loc[(dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['close'] < dataframe['sma_75'] * self.buy_37_ma_offset) & (dataframe['ewo'] > self.buy_37_ewo) & (dataframe['rsi'] < self.buy_37_rsi) & (dataframe['cti'] < self.buy_37_cti), ['enter_long', 'enter_tag']] = (1, 'NFINext37')
        dataframe.loc[(dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.buy_ema_open_mult_7) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['cti'] < self.buy_cti_7), ['enter_long', 'enter_tag']] = (1, 'NFINext7')
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe['fisher'] > self.sell_fisher.value) & dataframe['ha_high'].le(dataframe['ha_high'].shift(1)) & dataframe['ha_high'].shift(1).le(dataframe['ha_high'].shift(2)) & dataframe['ha_close'].le(dataframe['ha_close'].shift(1)) & (dataframe['ema_fast'] > dataframe['ha_close']) & (dataframe['ha_close'] * self.sell_bbmiddle_close.value > dataframe['bb_middleband']) & (dataframe['volume'] > 0), 'exit_long'] = 0
        return dataframe
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime, current_rate: float, current_profit: float, min_stake: float, max_stake: float, **kwargs):
        ...
        if count_of_buys == 1 and last_candle['tpct_change_0'] > 0.018 and (last_candle['close'] < last_candle['open']):
            ...
            return None
        elif count_of_buys == 5 and last_candle['cmf_1h'] < 0.0 and (last_candle['close'] < last_candle['open']) and (last_candle['rsi_14_1h'] < 30) and (last_candle['tpct_change_0'] > 0.018) and (last_candle['close'] < last_candle['open']) and (last_candle['ema_vwap_diff_50'] < 0.215) and (last_candle['ema_5'] >= last_candle['ema_10']):
            ...
            return None
        elif count_of_buys == 6 and last_candle['cmf_1h'] < 0.0 and (last_candle['close'] < last_candle['open']) and (last_candle['rsi_14_1h'] < 30) and (last_candle['tpct_change_0'] > 0.018) and (last_candle['close'] < last_candle['open'] and last_candle['ema_vwap_diff_50'] < 0.215) and (last_candle['ema_5'] >= last_candle['ema_10']):
            logger.info(f'DCA for {trade.pair} waiting for cmf_1h ({last_candle['cmf_1h']}) to rise above 0. Waiting for rsi_1h ({last_candle['rsi_14_1h']})to rise above 30')
            return None
        elif count_of_buys == 7 and last_candle['cmf_1h'] < 0.0 and (last_candle['close'] < last_candle['open']) and (last_candle['rsi_14_1h'] < 30) and (last_candle['tpct_change_0'] > 0.018) and (last_candle['close'] < last_candle['open'] and last_candle['ema_vwap_diff_50'] < 0.215) and (last_candle['ema_5'] >= last_candle['ema_10']):
            logger.info(f'DCA for {trade.pair} waiting for cmf_1h ({last_candle['cmf_1h']}) to rise above 0. Waiting for rsi_1h ({last_candle['rsi_14_1h']})to rise above 30')
            return None
        elif count_of_buys == 8 and last_candle['cmf_1h'] < 0.0 and (last_candle['close'] < last_candle['open']) and (last_candle['rsi_14_1h'] < 30) and (last_candle['tpct_change_0'] > 0.018) and (last_candle['close'] < last_candle['open'] and last_candle['ema_vwap_diff_50'] < 0.215) and (last_candle['ema_5'] >= last_candle['ema_10']):
            logger.info(f'DCA for {trade.pair} waiting for cmf_1h ({last_candle['cmf_1h']}) to rise above 0. Waiting for rsi_1h ({last_candle['rsi_14_1h']})to rise above 30')
            return None
...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    if MAtype == 1:
        ...
        mavalue = ta.zema(df, period=length)
