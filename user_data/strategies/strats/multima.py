...
from freqtrade.strategy import merge_informative_pair, DecimalParameter, IntParameter, BooleanParameter, timeframe_to_minutes
...
class MultiMA_TSL3(IStrategy):
    ...
    TRADE_OPEN_DATE = 4
    SELLMA_VALID = 5
    buy_params = {'base_nb_candles_buy_trima': 15, 'base_nb_candles_buy_trima2': 38, 'low_offset_trima': 0.959, 'low_offset_trima2': 0.949, 'base_nb_candles_buy_ema': 9, 'base_nb_candles_buy_ema2': 75, 'low_offset_ema': 1.067, 'low_offset_ema2': 0.973, 'base_nb_candles_buy_zema': 25, 'base_nb_candles_buy_zema2': 53, 'low_offset_zema': 0.958, 'low_offset_zema2': 0.961, 'base_nb_candles_buy_hma': 70, 'base_nb_candles_buy_hma2': 12, 'low_offset_hma': 0.948, 'low_offset_hma2': 0.941, 'buy_condition_trima_enable': True, 'buy_condition_zema_enable': True, 'buy_condition_hma_enable': True, 'ewo_high': 2.615, 'ewo_high2': 2.188, 'ewo_low': -19.632, 'ewo_low2': -19.955, 'rsi_buy': 60, 'rsi_buy2': 45}
    sell_params = {'base_nb_candles_ema_sell': 5, 'high_offset_sell_ema': 0.994}
    # ROI table:
    minimal_roi = {'0': 100}
    stoploss = -0.25
    ...
    # value loaded from strategy
    # value loaded from strategy
    protection_params = {'low_profit_lookback': 48, 'low_profit_min_req': 0.04, 'low_profit_stop_duration': 14, 'cooldown_lookback': 2, 'stoploss_lookback': 72, 'stoploss_stop_duration': 20}
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if self.custom_info[pair][self.DATESTAMP] != last_candle['date']:
            ...
            # live or dry
            elif self.custom_info[pair][self.IN_TRADE] == 1:
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_condition_trima_enable.value:
            ...
            buy_offset_trima = (dataframe['close'] < dataframe['trima_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['trima_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh'])
        ...
        if self.buy_condition_zema_enable.value:
            ...
            buy_offset_zema = (dataframe['close'] < dataframe['zema_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['zema_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh'])
        ...
        if self.buy_condition_hma_enable.value:
            ...
            buy_offset_hma = ((dataframe['close'] < dataframe['hma_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & (dataframe['rsi'] < 35) | (dataframe['close'] < dataframe['hma_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & (dataframe['rsi'] < 30)) & (dataframe['rsi_fast'] < 30)
            dataframe.loc[buy_offset_hma, 'enter_tag'] += 'hma '
            conditions.append(buy_offset_hma)
        add_check = dataframe['live_data_ok'] & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_sell'] * self.high_offset_sell_ema.value) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < self.buy_rsi_fast.value) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & ((dataframe['close'] < dataframe['ema_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_buy.value)) | (dataframe['close'] < dataframe['ema_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low2.value) | (dataframe['ewo'] > self.ewo_high2.value) & (dataframe['rsi'] < self.rsi_buy2.value))) & (dataframe['volume'] > 0)
        if conditions:
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.custom_info[metadata['pair']][self.SELLMA_VALID] == 1 and self.dp.runmode.value in ('live', 'dry_run'):
            sell_cond_2 = (dataframe['close'] > dataframe['sellma_offset']) & (dataframe['volume'] > 0)
...
class MultiMA_TSL3a(MultiMA_TSL3):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_condition_hma_enable.value:
            ...
            dataframe.loc[buy_offset_hma, 'enter_tag'] += 'hma '
            conditions.append(buy_offset_hma)
        add_check = dataframe['live_data_ok'] & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_sell'] * self.high_offset_sell_ema.value) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < self.buy_rsi_fast.value) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & (dataframe['pump'].rolling(20).max() < 1) & ((dataframe['close'] < dataframe['ema_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_buy.value)) | (dataframe['close'] < dataframe['ema_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low2.value) | (dataframe['ewo'] > self.ewo_high2.value) & (dataframe['rsi'] < self.rsi_buy2.value))) & (dataframe['volume'] > 0)
        if conditions:
            dataframe.loc[add_check & reduce(lambda x, y: x | y, conditions), ['buy_copy', 'enter_long']] = (1, 1)
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        conditions = []
        dataframe.loc[:, 'exit_tag'] = ''
        sell_cond_1 = (dataframe['rsi_fast_15m'] > self.min_rsi_sell_15m) & (dataframe['rsi'] > self.min_rsi_sell) & (dataframe['volume'] > 0)
...
class MultiMA_TSL3b(IStrategy):
    ...
    def version(self) -> str:
        return 'v3b'
    INTERFACE_VERSION = 3
    buy_params = {'base_nb_candles_buy_trima': 17, 'base_nb_candles_buy_trima2': 37, 'low_offset_trima': 0.941, 'low_offset_trima2': 0.914, 'base_nb_candles_buy_ema': 80, 'base_nb_candles_buy_ema2': 79, 'low_offset_ema': 1.074, 'low_offset_ema2': 0.942, 'base_nb_candles_buy_zema': 62, 'base_nb_candles_buy_zema2': 73, 'low_offset_zema': 0.961, 'low_offset_zema2': 0.98, 'base_nb_candles_buy_hma': 80, 'base_nb_candles_buy_hma2': 75, 'low_offset_hma': 0.96, 'low_offset_hma2': 0.965, 'base_nb_candles_buy_vwma': 26, 'base_nb_candles_buy_vwma2': 16, 'low_offset_vwma': 0.949, 'low_offset_vwma2': 0.951, 'ewo_high': 5.8, 'ewo_high2': 6.0, 'ewo_low': -10.8, 'ewo_low2': -15.3, 'rsi_buy': 60, 'rsi_buy2': 45}
    sell_params = {'base_nb_candles_ema_sell': 6, 'high_offset_sell_ema': 0.991}
    # ROI table:
    minimal_roi = {'0': 100}
    stoploss = -0.25
    optimize_sell_ema = False
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_condition_vwma_enable.value:
            ...
            buy_offset_vwma = (dataframe['close'] < dataframe['vwma_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) | (dataframe['close'] < dataframe['vwma_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh'])
            dataframe.loc[buy_offset_vwma, 'enter_tag'] += 'vwma '
            conditions.append(buy_offset_vwma)
        add_check = dataframe['live_data_ok'] & dataframe['age_filter_ok_1d'] & (dataframe['close'] < dataframe['Smooth_HA_L']) & (dataframe['close'] < dataframe['ema_offset_sell']) & (dataframe['close'].rolling(288).max() >= dataframe['close'] * 1.1) & (dataframe['Smooth_HA_O'].shift(1) < dataframe['Smooth_HA_H'].shift(1)) & (dataframe['rsi_fast'] < self.buy_rsi_fast.value) & (dataframe['rsi_84'] < 60) & (dataframe['rsi_112'] < 60) & ((dataframe['close'] < dataframe['ema_offset_buy']) & (dataframe['pm'] <= dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low.value) | (dataframe['ewo'] > self.ewo_high.value) & (dataframe['rsi'] < self.rsi_buy.value)) | (dataframe['close'] < dataframe['ema_offset_buy2']) & (dataframe['pm'] > dataframe['pmax_thresh']) & ((dataframe['ewo'] < self.ewo_low2.value) | (dataframe['ewo'] > self.ewo_high2.value) & (dataframe['rsi'] < self.rsi_buy2.value))) & (dataframe['volume'] > 0)
        if conditions:
            dataframe.loc[add_check & reduce(lambda x, y: x | y, conditions), 'enter_long'] = 1
        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[:, 'exit_tag'] = ''
        conditions = []
        sell_cond_1 = (dataframe['close'] > dataframe['ema_offset_sell']) & (dataframe['volume'] > 0) & (dataframe['rsi'] > self.min_rsi_sell.value)
        conditions.append(sell_cond_1)
