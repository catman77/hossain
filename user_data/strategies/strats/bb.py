...
class BB(IStrategy):
    ...
    ##
    ##
    buy_params = {'max_slip': 0.668, 'buy_bb_width_1h': 1.074, 'buy_roc_1h': 4, 'buy_threshold': 0.003, 'buy_bb_factor': 0.999, 'buy_bb_delta': 0.025, 'buy_bb_width': 0.095, 'buy_cci': -116, 'buy_cci_length': 25, 'buy_rmi': 49, 'buy_rmi_length': 17, 'buy_srsi_fk': 32, 'buy_closedelta': 13.494, 'buy_ema_diff': 0.024, 'buy_ema_high': 0.968, 'buy_ema_low': 0.935, 'buy_ewo': -5.001, 'buy_rsi': 23, 'buy_rsi_fast': 44, 'buy_ema_high_2': 1.087, 'buy_ema_low_2': 0.97, 'buy_ewo_high_2': 4.179, 'buy_rsi_ewo_2': 35, 'buy_rsi_fast_ewo_2': 45, 'buy_closedelta_local_dip': 13.717, 'buy_ema_diff_local_dip': 0.024, 'buy_ema_high_local_dip': 1.084, 'buy_rsi_local_dip': 20, 'buy_r_deadfish_bb_factor': 0.911, 'buy_r_deadfish_bb_width': 0.091, 'buy_r_deadfish_ema': 0.972, 'buy_r_deadfish_volume_factor': 1.008, 'buy_r_deadfish_cti': -0.115, 'buy_r_deadfish_r14': -44.34, 'buy_clucha_bbdelta_close': 0.04, 'buy_clucha_bbdelta_tail': 0.913, 'buy_clucha_close_bblower': 0.04, 'buy_clucha_closedelta_close': 0.05, 'buy_clucha_rocr_1h': 0.416, 'buy_adx': 13, 'buy_cofi_39_r14': -85.016, 'buy_cofi_cti': -0.892, 'buy_ema_cofi': 1.147, 'buy_ewo_high': 8.594, 'buy_fastd': 28, 'buy_fastk': 39, 'buy_nfix_39_cti': -0.105, 'buy_nfix_39_r14': -81.827}
    # sell space
    ##
    ##
    sell_params = {'sell_cmf': -0.046, 'sell_ema': 0.988, 'sell_ema_close_delta': 0.022, 'sell_deadfish_profit': -0.05, 'sell_deadfish_bb_factor': 0.954, 'sell_deadfish_bb_width': 0.043, 'sell_deadfish_volume_factor': 2.37}
    minimal_roi = {'0': 0.205}
    timeframe = '5m'
    inf_1h = '1h'
    ...
    buy_ema_high_local_dip = DecimalParameter(0.9, 1.2, default=0.942, optimize=is_optimize_local_dip)
    ...
    buy_ema_low_2 = DecimalParameter(0.9, 1.2, default=0.97, optimize=is_optimize_ewo_2)
    buy_ema_high_2 = DecimalParameter(0.9, 1.2, default=1.087, optimize=is_optimize_ewo_2)
    ...
    buy_r_deadfish_ema = DecimalParameter(0.9, 1.2, default=1.087, optimize=is_optimize_r_deadfish)
    buy_r_deadfish_bb_width = DecimalParameter(0.03, 0.75, default=0.05, optimize=is_optimize_r_deadfish)
    buy_r_deadfish_bb_factor = DecimalParameter(0.9, 1.2, default=1.0, optimize=is_optimize_r_deadfish)
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float, current_profit: float, **kwargs):
        ...
        if current_profit < -0.05 and last_candle['close'] < last_candle['ema_200'] * self.sell_ema.value and (last_candle['cmf'] < self.sell_cmf.value) and ((last_candle['ema_200'] - last_candle['close']) / last_candle['close'] < self.sell_ema_close_delta.value) and (last_candle['rsi'] > previous_candle_1['rsi']) and (last_candle['rsi'] > last_candle['rsi_1h'] + self.sell_rsi_delta.value):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        is_dip = (dataframe[f'rmi_length_{self.buy_rmi_length.value}'] < self.buy_rmi.value) & (dataframe[f'cci_length_{self.buy_cci_length.value}'] <= self.buy_cci.value) & (dataframe['srsi_fk'] < self.buy_srsi_fk.value)  # from BinH
        is_break = (dataframe['bb_delta'] > self.buy_bb_delta.value) & (dataframe['bb_width'] > self.buy_bb_width.value) & (dataframe['closedelta'] > dataframe['close'] * self.buy_closedelta.value / 1000) & (dataframe['close'] < dataframe['bb_lowerband3'] * self.buy_bb_factor.value)  # from NFI next gen
        is_local_uptrend = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.buy_ema_diff.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['bb_lowerband2'] * self.buy_bb_factor.value) & (dataframe['closedelta'] > dataframe['close'] * self.buy_closedelta.value / 1000)
        is_local_dip = (dataframe['ema_26'] > dataframe['ema_12']) & (dataframe['ema_26'] - dataframe['ema_12'] > dataframe['open'] * self.buy_ema_diff_local_dip.value) & (dataframe['ema_26'].shift() - dataframe['ema_12'].shift() > dataframe['open'] / 100) & (dataframe['close'] < dataframe['ema_20'] * self.buy_ema_high_local_dip.value) & (dataframe['rsi'] < self.buy_rsi_local_dip.value) & (dataframe['crsi'] > self.buy_crsi_local_dip.value) & (dataframe['closedelta'] > dataframe['close'] * self.buy_closedelta_local_dip.value / 1000)  # from SMA offset
        ...
        is_ewo_2 = (dataframe['ema_200_1h'] > dataframe['ema_200_1h'].shift(12)) & (dataframe['ema_200_1h'].shift(12) > dataframe['ema_200_1h'].shift(24)) & (dataframe['rsi_fast'] < self.buy_rsi_fast_ewo_2.value) & (dataframe['close'] < dataframe['ema_8'] * self.buy_ema_low_2.value) & (dataframe['EWO'] > self.buy_ewo_high_2.value) & (dataframe['close'] < dataframe['ema_16'] * self.buy_ema_high_2.value) & (dataframe['rsi'] < self.buy_rsi_ewo_2.value)  # reverse deadfish
        is_r_deadfish = (dataframe['ema_100'] < dataframe['ema_200'] * self.buy_r_deadfish_ema.value) & (dataframe['bb_width'] > self.buy_r_deadfish_bb_width.value) & (dataframe['close'] < dataframe['bb_middleband2'] * self.buy_r_deadfish_bb_factor.value) & (dataframe['volume_mean_12'] > dataframe['volume_mean_24'] * self.buy_r_deadfish_volume_factor.value) & (dataframe['cti'] < self.buy_r_deadfish_cti.value) & (dataframe['r_14'] < self.buy_r_deadfish_r14.value)
        is_clucHA = (dataframe['rocr_1h'] > self.buy_clucha_rocr_1h.value) & ((dataframe['bb_lowerband2_40'].shift() > 0) & (dataframe['bb_delta_cluc'] > dataframe['ha_close'] * self.buy_clucha_bbdelta_close.value) & (dataframe['ha_closedelta'] > dataframe['ha_close'] * self.buy_clucha_closedelta_close.value) & (dataframe['tail'] < dataframe['bb_delta_cluc'] * self.buy_clucha_bbdelta_tail.value) & (dataframe['ha_close'] < dataframe['bb_lowerband2_40'].shift()) & (dataframe['ha_close'] < dataframe['ha_close'].shift()) | (dataframe['ha_close'] < dataframe['ema_slow']) & (dataframe['ha_close'] < self.buy_clucha_close_bblower.value * dataframe['bb_lowerband2']))
        is_cofi = (dataframe['open'] < dataframe['ema_8'] * self.buy_ema_cofi.value) & qtpylib.crossed_above(dataframe['fastk'], dataframe['fastd']) & (dataframe['fastk'] < self.buy_fastk.value) & (dataframe['fastd'] < self.buy_fastd.value) & (dataframe['adx'] > self.buy_adx.value) & (dataframe['EWO'] > self.buy_ewo_high.value) & (dataframe['cti'] < self.buy_cofi_cti.value) & (dataframe['r_14'] < self.buy_cofi_39_r14.value)
        # NFI quick mode
        ...
        is_additional_check = (dataframe['roc_1h'] < self.buy_roc_1h.value) & (dataframe['bb_width_1h'] < self.buy_bb_width_1h.value)
        ## Additional Check
